...
function context(storageWorks=true){
  ...
  const c=vm.createContext({ResizeObserver:class{observe(){}disconnect(){}},document:doc,window,localStorage:{getItem(){if(!storageWorks)throw Error('blocked');return stored;},setItem(k,v){if(!storageWorks)throw Error('blocked');stored=v;}},setTimeout:()=>1,clearTimeout(){},console,performance:{now:()=>0},navigator:{},CustomEvent:class{},RoomAudio:class{unlock(){return Promise.resolve();}setState(){}setMuted(){}updateMix(){}},DeathCinematic:class{prepare(){}play(){}stop(){}},Engine:class{static getMissingFeatures(){return [];}startGame(){return Promise.resolve();}},AbortController,structuredClone});
  vm.runInContext(viewport+'\n'+script+'\n'+ui,c);return {c,doc,window,commands,$:doc.getElementById};
}
const snapshot=(phase,extra={})=>({phase,room:1,life:3,time:0,kills:0,level:1,xp:0,need:22,alive:22,weapon:'slipper',weapon_name:'拖鞋',weapon_icon:'▰',runes:[],combos:[],parts:0,...extra});
{
  ...
  t.$('story-screen').querySelector('[data-story-skip]').onclick();assert.equal(t.commands.length,1);assert.deepEqual(t.commands[0],{action:'start'});checks++;
  t.$('story-screen').querySelector('video').emit('ended');assert.equal(t.commands.length,1);checks++;
  t.$('restart').onclick();assert.deepEqual(t.commands.at(-1),{action:'start'});assert.equal(t.$('story-screen').hidden,true);checks++;
  ...
  t.window.receiveGameState(snapshot('win',{campaign_complete:true,room:5,campaign_rooms:5}));assert.equal(t.$('win-title').textContent,'你回到了现实。');t.$('next').onclick();assert.deepEqual(t.commands.at(-1),{action:'start'});checks++;
  t.window.receiveGameState(snapshot('playing'));t.window.receiveGameState(snapshot('win'));t.$('next').onclick();assert.deepEqual(t.commands.at(-1),{action:'next'});checks++;
}
{
  const t=context(false);t.window.gameEvent({kind:'ready'});t.$('start').onclick();t.$('story-screen').querySelector('video').emit('error');assert.deepEqual(t.commands.at(-1),{action:'start'});checks++;
}
{
  ...
  t.$('story-screen').querySelector('[data-story-skip]').onclick();assert.deepEqual(t.commands.at(-1),{action:'start'});checks++;
}
